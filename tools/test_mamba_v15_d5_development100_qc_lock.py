#!/usr/bin/env python3
"""Contract tests for the D5 development100 final QC lock."""
...
from lock_mamba_v15_d5_development100_qc import (
    duplicate_groups,
    overlap_groups,
    verify_protocol,
    verify_sealed_empty,
...
def main() -> None:
    protocol_path = Path(
        "docs/mamba_v15_d5_development100_final_qc_lock_protocol_v1.json"
    )
    ...
    print("[ok] D5 final-lock protocol and permission boundary are fixed")
    print("[ok] duplicate and prior-overlap hard gates are explicit")
    print("[ok] sealed partitions and immutable output behavior are enforced")
    print("[locked] generation=false model=false training=false selection=false")
