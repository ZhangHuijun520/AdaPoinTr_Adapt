#!/usr/bin/env python3
"""Audit frozen D4 M2 outputs without authorizing training or selection."""
...
AUDIT_PROTOCOL_ID = "mamba-v14-d4-mug500plus-m2-generation-audit-v1"
GENERATION_PROTOCOL_ID = "mamba-v14-d4-mug500plus-m2-fourfold-v1"
EXPECTED_GENERATOR_SHA256 = (
    "4ac9b1cb29f46e79e5dde1adfd8abf868e8a440dd366e25237bafcc5369c7e93"
)
EXPECTED_FOURFOLD_MANIFEST_SHA256 = (
    "3b431cfddc3c9575f9269e1535bd3c4ba66c1220b64a77b14256fea152aa1b7b"
)
EXPECTED_ASSIGNMENT_SHA256 = (
    "94b74c54267bed307e84d9e64700e6606f2605bbec72854184e618cbe3361735"
)
EXPECTED_AUDIT_PROTOCOL_SHA256 = (
    "428313d35df014dab835bd474ef92cddc2d95fd988a9762e5eb3d26619117901"
)
EXPECTED_FAMILIES = {
    "ellipsoid_small",
    "ellipsoid_medium",
...
def validate_audit_protocol(protocol: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        or protocol.get("status") != "preregistered_before_generation_audit"
        ...
        or effect.get("D4_training_authorized_on_pass") is not False
        or effect.get("D4_candidate_selection_authorized_on_pass") is not False
        or effect.get("protected_data_access_authorized") is not False
    ):
        raise RuntimeError("D4 generation-audit protocol semantics are invalid")
...
def validate_generation_receipt(
    ...
    if (
        ...
        != "generated_training_locked_pending_D4_generation_audit"
        or receipt.get("generator_sha256") != EXPECTED_GENERATOR_SHA256
        or receipt.get("source100_files_manifest_sha256")
        != EXPECTED_SOURCE_MANIFEST_SHA256
        or receipt.get("source100_receipt_sha256")
        != EXPECTED_SOURCE_RECEIPT_SHA256
        or receipt.get("source100_assets_sha256")
        != EXPECTED_SOURCE_ASSETS_SHA256
        ...
        or receipt.get("protected_data_used") is not False
    ):
        raise RuntimeError("D4 generation receipt semantics are invalid")


def read_fold_lock(directory: Path) -> Dict[str, str]:
    if sha256_file(directory / "files.sha256") != EXPECTED_FOURFOLD_MANIFEST_SHA256:
        raise RuntimeError("Frozen D4 fourfold files.sha256 drifted")
    ...
    if sha256_file(assignment) != EXPECTED_ASSIGNMENT_SHA256:
        raise RuntimeError("Frozen D4 source-fold assignments drifted")
    ...
    if len(rows) != 100 or len(result) != 100 or set(result.values()) != set("ABCD"):
        raise RuntimeError("Frozen D4 source-fold assignments are invalid")
    counts = collections.Counter(result.values())
    if counts != collections.Counter({fold: 25 for fold in "ABCD"}):
        raise RuntimeError("Frozen D4 source-fold counts are invalid")
    return result
...
def write_locked(files: Dict[str, bytes], output_dir: Path) -> None:
    if output_dir.exists():
        ...
        if existing != files:
            raise RuntimeError("Refusing to overwrite a non-identical D4 audit")
        print(f"[locked] existing D4 generation audit is byte-identical: {output_dir}")
        return
    ...
    print(f"[saved] immutable D4 generation audit: {output_dir}")
...
def main() -> None:
    ...
    if sha256_bytes(audit_protocol_bytes) != EXPECTED_AUDIT_PROTOCOL_SHA256:
        raise RuntimeError("Frozen D4 generation-audit protocol drifted")
    ...
    if len(rows) != 400:
        raise RuntimeError("D4 manifest must contain exactly 400 records")

    source_rows = read_source_lock(args.source100_qc_lock_dir.resolve())
    source_by_id = {row["case_id"].upper(): row for row in source_rows}
    if len(source_by_id) != 100:
        raise RuntimeError("D4 source100 lock must contain exactly 100 sources")
    ...
    source_data_root = args.source100_qc_lock_dir.resolve().parent.parent
    ...
    for row in tqdm(rows, desc="Audit D4 M2 generation"):
        ...
        if row.get("d4_fold") != expected_fold:
            raise RuntimeError(f"{case_id}: D4 fold differs from frozen assignment")
        if row.get("generator_sha256") != EXPECTED_GENERATOR_SHA256:
            raise RuntimeError(f"{case_id}: generator SHA256 differs")
        if row.get("source_dataset") != "mug500plus-v20-d4-source100":
    ...
    if len(case_ids) != 400 or len(derived_hashes) != 400:
        raise RuntimeError("D4 case IDs and hashes must be unique")
    if len(by_source) != 100 or len(source_hash_cache) != 100:
        raise RuntimeError("D4 audit must bind exactly 100 source skulls")
    for source_id, items in by_source.items():
        if (
            len(items) != 4
            or {str(item["defect_type"]) for item in items} != EXPECTED_FAMILIES
            or {str(item["d4_fold"]) for item in items} != {fold_by_source[source_id]}
    ...
    if fold_case_counts != expected_fold_counts:
        raise RuntimeError("D4 fold case counts are invalid")
    ...
    summary = {
        ...
        "status": "generation_integrity_passed_training_and_selection_still_locked",
        "audit_protocol_sha256": EXPECTED_AUDIT_PROTOCOL_SHA256,
        "implementation_hashes": {
            "audit": sha256_file(Path(__file__).resolve()),
            "tests": sha256_file(
                Path(__file__).with_name(
                    "test_mamba_v14_d4_mug500plus_m2_generation_audit.py"
                )
            ),
        },
        ...
        "next_step": "freeze_a_separate_D4_candidate_and_training_protocol",
    }
    report = (
        "# Mamba v1.4 D4 MUG500+ M2 生成完整性审计\n\n"
        "> 本审计只验证冻结派生数据，不训练模型、不选择候选、不访问保护数据。\n\n"
        "- source skull：100。\n"
        "- derived case：400。\n"
        "- 四折病例：A/B/C/D 各 100。\n"
        "- 每来源四种缺损族：全部完整且同折。\n"
    ...
    print("[ok] D4 M2 generation integrity passed: 100 sources / 400 cases")
    print(f"[saved] portable manifest: {output_dir / 'manifest_portable.jsonl'}")
    print("[locked] D4 training=false selection=false protected=false")
    print("[next] freeze a separate D4 candidate and training protocol")
