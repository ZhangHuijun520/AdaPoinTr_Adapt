#!/usr/bin/env python3
"""Boundary tests for the D6 development generation/fourfold protocol."""
...
from generate_mamba_v16_d6_mug500plus_development_cases import (
...
PROTOCOL_PATH = (
    ROOT
    / "docs"
    / "mamba_v16_d6_mug500plus_development_generation_fourfold_protocol_v1.json"
)
BASE_PROTOCOL_PATH = (
    ROOT
    / "docs"
...
def main() -> None:
    ...
    placeholders = {
        ...
        / "tools"
        / "generate_mamba_v16_d6_mug500plus_development_cases.py",
        "protocol_locker": ROOT
        / "tools"
        / "lock_mamba_v16_d6_mug500plus_development_fourfold_protocol.py",
        "tests": Path(__file__),
        "zero_step_report": ROOT
        / "docs"
        / "mamba_v16_d6a_slot32_implementation_zero_step_complete_result_zh.md",
    }
    ...
    assert first == second
    receipt = json.loads(first["d6_development_protocol_lock_receipt.json"])
    assert receipt["counts"]["planned_development_cases"] == 400
    assert receipt["D6_development_synthetic_generation_authorized_next"] is True
    assert receipt["D6A_R0_R1_implementation_frozen"] is True
    assert receipt["D6_gradient_calibration_authorized"] is False
    assert receipt["D6A_training_authorized"] is False
    ...
    permissive["lock_effect"]["D6A_training_authorized"] = True
    ...
    print("[ok] D6 inherits the frozen M2 v1 geometry and sampling sections")
    ...
    print("[locked] generation_not_started=true calibration=false training=false confirmation=false")
