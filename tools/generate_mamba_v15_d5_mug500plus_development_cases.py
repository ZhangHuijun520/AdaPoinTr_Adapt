#!/usr/bin/env python3
"""Generate the frozen D5 development400 cases; never touches sealed partitions."""
...
def build_effective_protocol(
    d5_protocol: Dict[str, Any], base_protocol: Dict[str, Any]
) -> Dict[str, Any]:
    validate_protocol(d5_protocol)
    engine = d5_protocol["lineage"]["m2_v1_engine"]
    ...
    generation = d5_protocol["development_generation"]
    ...
    effective["forbidden"] = sorted(
        set(effective.get("forbidden", [])) | set(d5_protocol["forbidden"])
    )
    if tuple(effective["defect_families"]) != EXPECTED_DEFECT_TYPES:
        raise RuntimeError("Effective D5 defect ordering differs from the frozen contract")
    return effective
...
def verify_protocol_lock(directory: Path, protocol_bytes: bytes) -> Dict[str, str]:
    ...
    receipt = json.loads(
        (directory / "d5_development_protocol_lock_receipt.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    if (
        ...
        != EXPECTED_BASE_PROTOCOL_SHA256
        or receipt.get("D5_development_synthetic_generation_authorized_next")
        is not True
        ...
        or receipt.get("D5_candidate_selection_authorized") is not False
        ...
        raise RuntimeError("D5 protocol-lock semantics are invalid")
    with (directory / "source_fold_assignments.csv").open(
        newline="", encoding="utf-8"
    ) as handle:
        rows = list(csv.DictReader(handle))
    if len(rows) != 100 or len({row["source_id"] for row in rows}) != 100:
        raise RuntimeError("D5 source-fold assignment lock is invalid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--d5_protocol_json", type=Path, required=True)
...
def main() -> None:
    args = parse_args()
    protocol_bytes = args.d5_protocol_json.read_bytes()
    d5_protocol = json.loads(protocol_bytes)
    ...
    effective = build_effective_protocol(d5_protocol, base_protocol)
    ...
    generator_hash = bundle_sha256(
        [Path(__file__), engine_path, args.d5_protocol_json, args.base_protocol_json]
    )
    print(f"[ok] D5 development sources={len(source_paths)}")
    print(f"[sha256] D5 generator_bundle={generator_hash}")
    ...
    if workers == 1:
        for task in tqdm(tasks, desc="D5 development source skulls"):
        ...
        with ProcessPoolExecutor(max_workers=workers) as executor:
            ...
            for future in tqdm(
                ...
                total=len(futures),
                desc="D5 development source skulls",
            ):
                records.extend(future.result())
    records.sort(key=lambda row: (row["skull_id"], row["defect_type"]))
    if len(records) != 400:
        raise RuntimeError(f"Expected 400 D5 development cases, got {len(records)}")
    for record in records:
        source_id = record["skull_id"].removeprefix("mug500plus__")
        record["d5_fold"] = fold_by_source[source_id]
        record["d5_partition"] = "development"
    manifest = args.out_dir / "manifest.jsonl"
    write_jsonl(manifest, records)
    receipt = {
        "protocol_id": d5_protocol["protocol_id"],
    ...
    print("[locked] model=false training=false selection=false sealed=false pending_audit=true")
