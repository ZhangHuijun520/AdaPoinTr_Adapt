#!/usr/bin/env python3
"""Verify, extract, and freeze one D6 development source QC batch."""
...
EXPECTED_PROTOCOL_SHA256 = (
    "12fea0f2279e5d4aea3fdac3cb8fdcf70ba0645524d1b962e43c6f4babf4d894"
)
...
def verify_hash_manifest(root: Path) -> None:
    ...
    for line in manifest.read_text(encoding="ascii").splitlines():
        ...
        if not path.is_file() or sha256_file(path) != expected.lower():
            raise RuntimeError(f"Frozen D6 source-lock hash mismatch: {path}")
...
def read_batch_contract(
    source_lock_dir: Path, zero_step_report: Path, batch_id: int
    ...
    if sha256_file(receipt_path) != EXPECTED_LOCK_RECEIPT_SHA256:
        raise RuntimeError("Unexpected D6 source125 lock receipt")
    if sha256_file(source_lock_dir / "files.sha256") != EXPECTED_LOCK_MANIFEST_SHA256:
        raise RuntimeError("Unexpected D6 source125 lock manifest")
    if sha256_file(zero_step_report) != EXPECTED_ZERO_STEP_REPORT_SHA256:
        raise RuntimeError("Unexpected D6-A zero-step parent result")
    receipt = json.loads(receipt_path.read_text(encoding="utf-8"))
    if (
        receipt.get("status") != "source125_terminal_two_partition_acquisition_locked"
        or receipt.get("source_overlap") != 0
        or receipt.get("development_archive_download_authorized_next") is not True
        or receipt.get("development_extraction_authorized") is not False
        or receipt.get("proposal_confirmation_extraction_authorized") is not False
        or receipt.get("protected_data_accessed") is not False
        or receipt.get("D6_training_authorized") is not False
        or receipt.get("D6_confirmation_access_authorized") is not False
    ):
        raise RuntimeError("D6 source125 acquisition boundary drifted")
    ...
    source_plan = read_csv(source_lock_dir / "d6_source125_source_plan.csv")
    by_id = {row["source_id"]: row for row in source_plan}
    if len(by_id) != len(source_plan) or len(set(expected_ids)) != len(expected_ids):
        raise RuntimeError("D6 source plan or batch IDs contain duplicates")
    try:
    ...
    if any(
        ...
        or int(row.get("batch_id", "0")) != batch_id
        for row in sources
    ):
...
def verify_archives(plan: Sequence[Dict[str, str]], archive_dir: Path) -> None:
    ...
    for row in tqdm(plan, desc="Verify D6 development ZIPs"):
...
def extract_batch(
    ...
    try:
        for row in tqdm(plan, desc="Extract frozen D6 clear STL"):
...
def qc_rows_for_sources(
    ...
    rows = [
        ...
        for row in tqdm(sources, desc="D6 development geometry QC")
    ]
...
def freeze_qc(
    ...
    summary = {
        "protocol_id": "mamba-v16-d6-development-batch-qc-v1",
        "batch_id": batch_id,
    ...
    receipt = {
        ...
        "d6a_zero_step_complete_report_sha256": EXPECTED_ZERO_STEP_REPORT_SHA256,
        ...
        "D6_synthetic_generation_authorized": False,
    ...
    report = (
        "# Mamba v1.6 D6 Development Batch QC\n\n"
        "> 仅提取冻结 development clear STL；不读取 confirmation25，不运行模型。\n\n"
    ...
    outputs = {
        ...
        "d6_development_source_qc_per_case.csv": csv_bytes(list(qc_rows[0]), qc_rows),
...
def main() -> None:
    ...
    protocol = (
        ...
        / "docs"
        / "mamba_v16_d6_development_batch_qc_protocol_v1.json"
    )
    if sha256_file(protocol) != EXPECTED_PROTOCOL_SHA256:
        raise RuntimeError("Frozen D6 development batch-QC protocol drifted")
    ...
    plan, sources, _ = read_batch_contract(
        args.source_lock_dir, args.zero_step_report, args.batch_id
    )
    ...
    print("[locked] confirmation=empty model=false generation=false training=false")
