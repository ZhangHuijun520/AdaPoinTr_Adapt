#!/usr/bin/env python3
"""Audit frozen D6 development400 outputs without widening permissions."""
...
AUDIT_PROTOCOL_ID = "mamba-v16-d6-mug500plus-development-generation-audit-v1"
...
EXPECTED_ASSIGNMENT_SHA256 = (
    "35d7b981395e4f630b1c2b071a42af38594380daa0ee92e60bb54653a6d0b5d1"
)
EXPECTED_AUDIT_PROTOCOL_SHA256 = (
    "5e8f09d9b5be5ea1271e4804c516701e72b69bf71f22cd96559a017e5ef901fc"
)
EXPECTED_FAMILIES = {
    "ellipsoid_small",
    "ellipsoid_medium",
...
def validate_audit_protocol(protocol: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        != "execution_protocol_frozen_after_generation_before_audit"
        ...
        or effect.get("D6A_R0_R1_implementation_remains_frozen") is not True
        or effect.get("D6_gradient_calibration_authorized_on_pass") is not False
        or effect.get("D6A_training_authorized_on_pass") is not False
        or effect.get("D6_seed1_authorized_on_pass") is not False
        or effect.get("D6B_training_authorized_on_pass") is not False
        or effect.get("D6_candidate_selection_authorized_on_pass") is not False
        ...
        raise RuntimeError("D6 generation-audit protocol semantics are invalid")
...
def validate_generation_receipt(
    ...
    if (
        ...
        raise RuntimeError("D6 generation receipt semantics are invalid")


def read_fold_lock(directory: Path) -> Dict[str, str]:
    if sha256_file(directory / "files.sha256") != EXPECTED_FOURFOLD_MANIFEST_SHA256:
        raise RuntimeError("Frozen D6 fourfold files.sha256 drifted")
    ...
    if sha256_file(assignment) != EXPECTED_ASSIGNMENT_SHA256:
        raise RuntimeError("Frozen D6 source-fold assignments drifted")
    ...
    if len(rows) != 100 or len(result) != 100 or set(result.values()) != set("ABCD"):
        raise RuntimeError("Frozen D6 source-fold assignments are invalid")
    counts = collections.Counter(result.values())
    if counts != collections.Counter({fold: 25 for fold in "ABCD"}):
        raise RuntimeError("Frozen D6 source-fold counts are invalid")
    return result
...
def write_locked(files: Dict[str, bytes], output_dir: Path) -> None:
    if output_dir.exists():
        ...
        if existing != files:
            raise RuntimeError("Refusing to overwrite a non-identical D6 audit")
        print(f"[locked] existing D6 generation audit is byte-identical: {output_dir}")
        return
    ...
    print(f"[saved] immutable D6 generation audit: {output_dir}")
...
def main() -> None:
    ...
    if sha256_bytes(audit_protocol_bytes) != EXPECTED_AUDIT_PROTOCOL_SHA256:
        raise RuntimeError("Frozen D6 generation-audit protocol drifted")
    ...
    if len(rows) != 400:
        raise RuntimeError("D6 manifest must contain exactly 400 records")

    source_rows, _ = read_lineage(
        args.development100_qc_lock_dir.resolve(),
        args.source125_acquisition_lock_dir.resolve(),
    )
    source_by_id = {row["case_id"].upper(): row for row in source_rows}
    if len(source_by_id) != 100:
        raise RuntimeError("D6 development100 lock must contain exactly 100 sources")
    ...
    for row in tqdm(rows, desc="Audit D6 development generation"):
        ...
        if row.get("d6_fold") != expected_fold:
            raise RuntimeError(f"{case_id}: D6 fold differs from frozen assignment")
        if row.get("d6_partition") != "development":
        ...
        if row.get("source_dataset") != "mug500plus-v20-d6-development100":
    ...
    if len(case_ids) != 400 or len(derived_hashes) != 400:
        raise RuntimeError("D6 case IDs and hashes must be unique")
    if len(by_source) != 100 or len(source_hash_cache) != 100:
        raise RuntimeError("D6 audit must bind exactly 100 source skulls")
    for source_id, items in by_source.items():
        if (
            len(items) != 4
            or {str(item["defect_type"]) for item in items} != EXPECTED_FAMILIES
            or {str(item["d6_fold"]) for item in items} != {fold_by_source[source_id]}
    ...
    if fold_case_counts != expected_fold_counts:
        raise RuntimeError("D6 fold case counts are invalid")
        ...
        "implementation_hashes": {
            ...
            "tests": sha256_file(
                Path(__file__).with_name(
                    "test_mamba_v16_d6_mug500plus_development_generation_audit.py"
                )
            ),
        },
    ...
    report = (
        "# Mamba v1.6 D6 development400 生成完整性审计\n\n"
        "> 本审计只执行生成协议中预先冻结的硬门控，不校准、不训练、不选择候选，不访问 proposal-confirmation25。\n\n"
        "- source skull：100。\n"
        "- derived case：400。\n"
        "- 四折病例：A/B/C/D 各 100。\n"
        "- 每来源四种缺损族：全部完整且同折。\n"
        ...
        "- 路径：全部为可解析相对路径。\n"
        "- proposal-confirmation25 与 official test：未访问。\n"
        "- 结论：生成完整性通过；D6 梯度校准、训练、seed-1、D6-B 与候选选择继续锁定。\n"
    )
    files = {
    ...
    print("[ok] D6 development generation integrity passed: 100 sources / 400 cases")
    ...
    print("[next] freeze a separate D6 candidate and training protocol")
