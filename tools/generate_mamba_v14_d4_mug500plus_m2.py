#!/usr/bin/env python3
"""Generate D4 M2 cases from the frozen source100 and fourfold protocol locks."""
...
def bundle_sha256(paths: List[Path]) -> str:
    ...
    for path in sorted((path.resolve() for path in paths), key=lambda path: path.name):
...
def build_effective_protocol(
    d4_protocol: Dict[str, Any], base_protocol: Dict[str, Any]
) -> Dict[str, Any]:
    validate_protocol(d4_protocol)
    engine = d4_protocol["lineage"]["m2_v1_engine"]
    ...
    overrides = d4_protocol["d4_generation_overrides"]
    effective["protocol_id"] = overrides["effective_protocol_id"]
    effective["status"] = "preregistered_generator_not_run"
    effective["determinism"]["master_seed"] = overrides["master_seed"]
    effective["derived_dataset"].update(
        {
            "expected_cases": overrides["expected_cases"],
    ...
    effective["forbidden"] = sorted(
        set(effective.get("forbidden", [])) | set(d4_protocol["forbidden"])
    )
    if tuple(effective["defect_families"]) != EXPECTED_DEFECT_TYPES:
        raise RuntimeError("Effective D4 defect ordering differs from the frozen contract")
    return effective


def verify_protocol_lock(directory: Path, d4_protocol_bytes: bytes) -> Dict[str, str]:
    verify_flat_manifest(directory, sha256_file(directory / "files.sha256"))
    receipt = json.loads(
        (directory / "d4_m2_protocol_lock_receipt.json").read_text(encoding="utf-8")
    )
    ...
    if (
        ...
        != hashlib.sha256(d4_protocol_bytes).hexdigest()
        ...
        != EXPECTED_BASE_PROTOCOL_SHA256
        or receipt.get("D4_M2_generation_authorized_next") is not True
        or receipt.get("D4_training_authorized") is not False
        or receipt.get("D4_candidate_selection_authorized") is not False
        or receipt.get("protected_data_accessed") is not False
    ):
        raise RuntimeError("D4 M2 protocol-lock semantics are invalid")
    with (directory / "source_fold_assignments.csv").open(
        newline="", encoding="utf-8"
    ) as handle:
        rows = list(csv.DictReader(handle))
    if len(rows) != 100 or len({row["source_id"] for row in rows}) != 100:
        raise RuntimeError("D4 M2 fold assignment lock is invalid")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--d4_protocol_json", type=Path, required=True)
...
def main() -> None:
    args = parse_args()
    d4_protocol_bytes = args.d4_protocol_json.read_bytes()
    d4_protocol = json.loads(d4_protocol_bytes)
    ...
    effective = build_effective_protocol(d4_protocol, base_protocol)
    fold_by_source = verify_protocol_lock(args.protocol_lock_dir, d4_protocol_bytes)
    lock_rows = read_source_lock(args.source100_qc_lock_dir)
    ...
    if set(source_paths) != set(fold_by_source):
        raise RuntimeError("Source100 lock and fourfold lock have different source IDs")
    generator_hash = bundle_sha256(
        [Path(__file__), engine_path, args.d4_protocol_json, args.base_protocol_json]
    )
    print(f"[ok] D4 sources={len(source_paths)}")
    print(f"[sha256] D4 generator_bundle={generator_hash}")
    ...
    print("[done] sources=100 cases=400")
    print("[locked] training=false selection=false protected=false pending_audit=true")
