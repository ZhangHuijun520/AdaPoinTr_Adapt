#!/usr/bin/env python
"""Compare identity overfit with fine coverage weights 1, 2, and 4."""
...
VARIANTS = (
    ...
    },
    {
        ...
        "query_selection": "fps_preserve",
        "denoise_weight": 0.0,
        "fine_coverage_weight": 2.0,
        "config": "cfgs/SkullFix_models/AdaPoinTr_identity_E_coverage2.yaml",
        "ckpt": (
            "experiments/AdaPoinTr_identity_E_coverage2/SkullFix_models/"
            "skullfix_identity_E_coverage2/ckpt-best.pth"
        ),
    },
    {
        "group": "F",
        "query_selection": "fps_preserve",
        "denoise_weight": 0.0,
        "fine_coverage_weight": 4.0,
        "config": "cfgs/SkullFix_models/AdaPoinTr_identity_F_coverage4.yaml",
        "ckpt": (
            "experiments/AdaPoinTr_identity_F_coverage4/SkullFix_models/"
            "skullfix_identity_F_coverage4/ckpt-best.pth"
        ),
    },
)
...
def make_row(result):
    ...
    return {
        "group": result["group"],
        "fine_coverage_weight": result["fine_coverage_weight"],
...
def main():
    ...
    print("==== SkullFix identity directional Chamfer comparison ====")
    print(
        "group coverage coarse_CD coarse_R->P fine_CD fine_HD95 "
        "fine_NSD@1 fine_P->R fine_R->P"
    )
    for row in rows:
        print(
            f"{row['group']:>5s} "
            f"{row['fine_coverage_weight']:>8.1f} "
