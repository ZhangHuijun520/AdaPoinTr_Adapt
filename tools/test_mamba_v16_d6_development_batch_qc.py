#!/usr/bin/env python3
"""Contract tests for D6 development-only extraction and QC."""
...
from qc_mamba_v16_d6_development_batch import index_clear_members, verify_sealed_empty
...
def main() -> None:
    protocol = json.loads(
        Path("docs/mamba_v16_d6_development_batch_qc_protocol_v1.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    assert (
        protocol["lock_effect"]["D6_confirmation_access_authorized"] is False
    )
    assert protocol["lock_effect"]["D6_synthetic_generation_authorized"] is False
    assert protocol["lock_effect"]["D6_R0_R1_training_authorized"] is False
    ...
    print("[ok] D6 accepts exactly one canonical clear STL per development source")
    print("[ok] unexpected or duplicate clear STL members are hard failures")
    print("[ok] confirmation25 must remain empty")
    print("[locked] model=false generation=false training=false confirmation=false")
