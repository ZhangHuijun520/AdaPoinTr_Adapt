...
CANDIDATE="${1:?Pass Q0, Q1, Q2, or Q3}"
FOLD="${2:?Pass fold A, B, C, or D}"
[[ "$CANDIDATE" =~ ^Q[0-3]$ ]] || { echo "[error] invalid candidate: $CANDIDATE"; exit 2; }
[[ "$FOLD" =~ ^[A-D]$ ]] || { echo "[error] invalid fold: $FOLD"; exit 2; }

ROOT="logs/skullbreak_mamba_v12_d21_geometry"
PROTOCOL="${ROOT}/protocol_v1/protocol_amendment.json"
CONFIG="cfgs/SkullBreak_models/generated_mamba_v12_d21_geometry_v1/MambaV12D21Geometry_${CANDIDATE}_fold${FOLD}_seed0.yaml"
CONFIG_STEM="$(basename "$CONFIG" .yaml)"
CONFIG_PARENT="$(basename "$(dirname "$CONFIG")")"
EXP_NAME="skullbreak_mamba_v12_d21_${CANDIDATE}_fold${FOLD}_seed0"
...
[[ -f "$PROTOCOL" && -f "$CONFIG" ]] || {
  echo "[error] D2.1 protocol/config missing; run prepare script first"
  exit 2
}
[[ ! -e "${RUN_DIR}/run_record.json" ]] || {
  echo "[locked] completed D2.1 run already exists: ${RUN_DIR}/run_record.json"
  exit 0
}
...
echo "[D2.1 Round A] candidate=${CANDIDATE} fold=${FOLD} seed=0"
echo "[locked] development84 reuse only; confirmation/monitor/official-test forbidden"
...
rm -f -- "$RAW_CKPT"
echo "[done] D2.1 Round A ${CANDIDATE} fold=${FOLD} seed=0 $(date)"
