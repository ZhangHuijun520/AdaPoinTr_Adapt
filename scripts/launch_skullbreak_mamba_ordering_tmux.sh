...
SESSION="${TMUX_SESSION:-mamba-ordering-v11-seed0}"
CANDIDATES="${CANDIDATES:-O0 O1 O2 O3}"
STAMP="$(date +%Y%m%d_%H%M%S)"
MASTER_LOG="${ROOT}/logs/skullbreak_mamba_ordering_v11_out8192/tmux_${STAMP}.log"
...
bash scripts/run_skullbreak_mamba_ordering_ablation_monitor.sh \
2>&1 | tee '$MASTER_LOG'; \
status=\${PIPESTATUS[0]}; \
