...
SESSION="${TMUX_SESSION:-mamba-v13-d3-s1-calibration}"
STAMP="$(date +%Y%m%d_%H%M%S)"
MASTER_LOG="${ROOT}/logs/mamba_v13_d3_mug500plus/s1_gradient_calibration_tmux/tmux_${STAMP}.log"
...
bash scripts/run_mamba_v13_d3_s1_calibration.sh \
2>&1 | tee '$MASTER_LOG'; \
status=\${PIPESTATUS[0]}; \
echo '[tmux] calibration exit status:' \$status; \
